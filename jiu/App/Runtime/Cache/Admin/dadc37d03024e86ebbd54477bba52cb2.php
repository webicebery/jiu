...
<article class="page-container">
    <form action="<?php echo U('Admin/Order/edit');?>" class="form form-horizontal" method="post" id="formstyle">
      
        <!-- 隐藏域接收ID -->
        <input type="hidden" name="id" value="<?php echo ($list[0][order_id]); ?>">
        <div class="row cl">
        <label class="form-label col-xs-4 col-sm-3">状态：</label>
        <div class="formControls col-xs-8 col-sm-9"> <span class="select-box" style="width:150px;">
            <?php if($list[0][order_status]=='0'): ?><select class="select" name="status" size="1">
                    <option value="3"}>关闭订单</option>
     
                </select>
            <?php elseif($list[0][order_status]=='4'): ?>
